...
class QueryBuilder extends Builder
{
    ...
    private function needFlushCache($type='u')
    {
        ...
        $realAutoEachCache = $this->formatAutoKey4Where($autoEachCache);#透明key，仅仅考虑where条件情况

        $flushKey = array_merge($realWhereFlushKeyArr, $realColumnsFlushKeyArr, $realWhereAutoFlushKey, $realColumnsAutoFlushKey, $realAutoEachCache);
        if( 0 == count($flushKey) ){
            return false;
        }
    ...
    public function insertGetId(array $values, $sequence = null)
    {
        ...
        $this->model->cleanColumnsArr();
        return parent::insertGetId($values, $sequence); // TODO: Change the autogenerated stub
    }
     ...
     */
    private function formatAutoKey4Where($cacheKeyArr){
        $realKeyArr = array();
     ...
     * @param $cacheKeys Array 缓存处理
     */
    private function formatAutoKey4Columns($cacheKeyArr){
        $realKeyArr = array();
        foreach($cacheKeyArr as $key => $value){
     ...
     */
    private function replaceKey($cacheKey, $type){
        $result = array();
        //匹配括号里面的值
